...
afterEach(async () => {
  delete process.env.FBH_CONFIG
  delete process.env.FBH_TUI_CONFIG
...
test("project config takes precedence over FBH_TUI_CONFIG (matches FBH_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.FBH_TUI_CONFIG = custom
    },
  })
...
test("FBH_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from FBH_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.FBH_CONFIG = path.join(customDir, "opencode.json")
    },
  })
