...
const FILES = [
  "AGENTS.md",
  ...(Flag.FBH_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.FBH_CONFIG_DIR) {
    files.push(path.join(Flag.FBH_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.FBH_DISABLE_CLAUDE_CODE_PROMPT) {
...
export namespace Instruction {
  ...
  export class Service extends Context.Service<Service, Interface>()("@foxybear/Instruction") {}
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          ...
          if (!Flag.FBH_CONFIG_DIR) {
            log.warn(
              `Skipping relative instruction "${instruction}" - no FBH_CONFIG_DIR set while project config is disabled`,
            )
            return []
          }
          return yield* fs
            .globUp(instruction, Flag.FBH_CONFIG_DIR, Flag.FBH_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
