...
import { createFbhClient, type FbhClient, type ToolPart } from "@foxybear/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run fbh with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running fbh server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to FBH_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: FbhClient) {
    ...
    async function share(sdk: FbhClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.FBH_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: FbhClient) {
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.FBH_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.FBH_SERVER_USERNAME ?? "foxybear"
        const auth = `Basic ${Buffer.from(`${username}:${password}`).toString("base64")}`
        return { Authorization: auth }
      })()
      const sdk = createFbhClient({ baseUrl: args.attach, directory, headers })
