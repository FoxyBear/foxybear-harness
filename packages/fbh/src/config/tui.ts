...
export namespace TuiConfig {
  ...
  export class Service extends Context.Service<Service, Interface>()("@foxybear/TuiConfig") {}
  ...
  async function loadState(ctx: { directory: string; worktree: string }) {
    let projectFiles = Flag.FBH_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.FBH_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", ctx.directory, ctx.worktree)
    ...
    const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.FBH_CONFIG_DIR)

    for (const dir of dirs) {
      if (!dir.endsWith(".opencode") && dir !== Flag.FBH_CONFIG_DIR) continue
